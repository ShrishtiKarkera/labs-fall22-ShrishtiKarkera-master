...
// so, while you can modify this code to help you debug, please test against the original before submitting.
//
...
type reqMsg struct { // when network is created, a channel of this struct is being created (shrishti) - line 130
...
func (e *ClientEnd) Call(svcMeth string, args interface{}, reply interface{}) bool {
	...
	req.args = qb.Bytes() // sending gob encoded args in req.args   (shrishti)

	e.ch <- req // this channel will be read somewhere (call me alpha) (shrishti)

	rep := <-req.replyCh // reading the sent reply from server (call me beta) (shrishti)
	if rep.ok {
...
type Network struct {
	mu             sync.Mutex // to avoid race conditions between requests (shrishti)
	reliable       bool
	longDelays     bool                        // pause a long time on send on disabled connection
	longReordering bool                        // sometimes delay replies a long time
	ends           map[interface{}]*ClientEnd  // ends, by name  (client name -> CliendEnd - shrishti)
	enabled        map[interface{}]bool        // by end name	(client name -> enable/disable - shrishti)
	servers        map[interface{}]*Server     // servers, by name   (server name -> Server - shrishti)
	connections    map[interface{}]interface{} // endname -> servername  (client name -> server name)
	endCh          chan reqMsg                 // client has a channel - used in Call too (shrishti)
}
...
func MakeNetwork() *Network {
	...
	rn.endCh = make(chan reqMsg) // dont understand this declaration (shrishti)

	// single goroutine to handle all ClientEnd.Call()s
	go func() {
		for xreq := range rn.endCh {
			go rn.ProcessReq(xreq) // gob encoded alpha sent to ProcessRed (shrishti)
		}
	}()
...
func (rn *Network) ReadEndnameInfo(endname interface{}) (enabled bool,
	...
	enabled = rn.enabled[endname] // endname is the client name (shrishti)
...
func (rn *Network) ProcessReq(req reqMsg) {
	...
	if enabled && servername != nil && server != nil {
		...
		go func() {
			r := server.dispatch(req) // sending the message beta after processsing alpha (shrishti)
			ech <- r
		}()
		...
		if replyOK == false || serverDead == true {
			...
			req.replyCh <- replyMsg{false, nil} // this is the beta reply message (shrishti)
